...
#define INPUT_SIZE    (4 * 4 * 256)
#define OUTPUT_SIZE   (2 * 2 * 512)  
#define WEIGHTS_1_SIZE  (3 * 3 * 256 * 512)
#define WEIGHTS_2_SIZE  (1 * 1 * 256 * 512)
#define BN_ALPHA_SIZE (512)
#define BN_BETA_SIZE  (512)
#define INPUT_PAD_1_SIZE  (5 * 5 * 256)
#define INPUT_PAD_2_SIZE  (4 * 4 * 256)
...
int run_bnn_sw(){
	...
	uint64_t *weights1 = (uint64_t *)current_addr;
    current_addr += (uint64_t)WEIGHTS_1_SIZE;
	uint64_t *weights2 = (uint64_t *)current_addr;
    current_addr += (uint64_t)WEIGHTS_2_SIZE;
	uint64_t *bn_alpha1 = (uint64_t *)current_addr;
    current_addr += (uint64_t)BN_ALPHA_SIZE;
    uint64_t *bn_beta1 = (uint64_t *)current_addr;
    current_addr += (uint64_t)BN_BETA_SIZE;
	uint64_t *bn_alpha2 = (uint64_t *)current_addr;
    current_addr += (uint64_t)BN_ALPHA_SIZE;
	uint64_t *bn_beta2 = (uint64_t *)current_addr;
    current_addr += (uint64_t)BN_BETA_SIZE;
	uint64_t *input_pad1 = (uint64_t *)current_addr;
	current_addr += (uint64_t)INPUT_PAD_1_SIZE;
	uint64_t *input_pad2 = (uint64_t *)current_addr;
	current_addr += (uint64_t)INPUT_PAD_2_SIZE;
	uint64_t *output = (uint64_t *)current_addr;

	memcpy(input, sw_1_4, sizeof(sw_1_4));
	memcpy(weights1, sw_w_1_1_4, sizeof(sw_w_1_1_4));
	memcpy(weights2, sw_w_2_1_4, sizeof(sw_w_2_1_4));
	memcpy(bn_alpha1, sw_ba_1_1_4, sizeof(sw_ba_1_1_4));
	memcpy(bn_beta1, sw_bb_1_1_4, sizeof(sw_bb_1_1_4));
	memcpy(bn_alpha2, sw_ba_2_1_4, sizeof(sw_ba_2_1_4));
	memcpy(bn_beta2, sw_bb_2_1_4, sizeof(sw_bb_2_1_4));

	uint64_t sw_tick = get_real_clock_tick();
	uint16_t batch_size = 1;
	uint16_t H = 4, W = 4, C = 256;
    uint16_t K = 3;
    uint16_t F = 512;
    uint16_t S = 2;
	...
	for (uint16_t h = 0; h < padded_h; h++) {
		for (uint16_t w = 0; w < padded_w; w++) {
			for (uint16_t c = 0; c < C; c++) {
				input_pad1[h*padded_w*C + w*C + c] = 0;
			}
		}
	}
	...
	for (uint16_t h = 0; h < H; h++) {
		for (uint16_t w = 0; w < W; w++) {
			for (uint16_t c = 0; c < C; c++) {
				input_pad1[(h+pad_top)*padded_w*C + (w+pad_left)*C + c] = input[h*W*C + w*C + c];
			}
		}
	}
                    ...
                    for (uint16_t kh = 0; kh < K; kh++) {
                        for (uint16_t kw = 0; kw < K; kw++) {
                            for (uint16_t c = 0; c < C; c++) {
								...
								int64_t input_val = (int64_t)input_pad1[input_h*padded_w*C + input_w*C + c];
								int64_t weight_val = (int64_t)weights1[kh*K*C*F + kw*C*F + c*F + f];
                                sum += input_val * weight_val;
                            }
                        }
                    }

					output[i*out_w*F + j*F + f] = ((sum*bn_alpha1[f] + bn_beta1[f]));
                }
            }
        }
	}
	...
	for (uint16_t h = 0; h < padded_h; h++) {
		for (uint16_t w = 0; w < padded_w; w++) {
			for (uint16_t c = 0; c < C; c++) {
				input_pad2[h*padded_w*C + w*C + c] = 0;
			}
		}
	}
	...
	for (uint16_t h = 0; h < H; h++) {
		for (uint16_t w = 0; w < W; w++) {
			for (uint16_t c = 0; c < C; c++) {
				input_pad2[(h+pad_top)*padded_w*C + (w+pad_left)*C + c] = input[h*W*C + w*C + c];
			}
		}
	}
                    ...
                    for (uint16_t kh = 0; kh < K; kh++) {
                        for (uint16_t kw = 0; kw < K; kw++) {
                            for (uint16_t c = 0; c < C; c++) {
								...
								int64_t input_val = (int64_t)input_pad2[input_h*padded_w*C + input_w*C + c];
								int64_t weight_val = (int64_t)weights2[kh*K*C*F + kw*C*F + c*F + f];
                                sum += input_val * weight_val;
                            }
                        }
                    }

					output[i*out_w*F + j*F + f] = ((int64_t)((sum*bn_alpha2[f] + bn_beta2[f])) + (int64_t)output[i*out_w*F + j*F + f]) >> 7;
                }
            }
        }
	}
	...
	int all_are_equal = 1;
	int mcmp = memory_compare(output, sw_o_1_4, sizeof(sw_o_1_4), 0);
	all_are_equal &= mcmp;
	if(all_are_equal){
		...
		for (size_t i = 0; i < 10; ++i){
			if(sw_o_1_4[i] != output[i]) {
				printf("\nidx %zu  EXPECT 0x%016llX  READ 0x%016llX", i, sw_o_1_4[i], output[i]);
			}
		}
	}
...
int main() {
	printf("SHORTCUT 1 TYPE ACCELERATOR\n");
	...
	memcpy(hw_mem_addr, mem_1_4, single_test_size);
	flush_cache();
	inst_param_t sc_pr;
